...
# Create fleet (grid of aliens)
rows, cols = 3, 6
for r in range(rows):
    for i in range(cols):
        targets.append(pygame.Rect(300+i*40, 50+r*40, 30, 30))

fleet_dir = 1  # 1 = right, -1 = left
...
while True:
    ...
    # Show score and aliens left
    s.blit(f.render(f"Score:{score}",1,(0,0,0)),(10,10))
    s.blit(f.render(f"Aliens left:{len(targets)}",1,(0,0,0)),(10,40))
